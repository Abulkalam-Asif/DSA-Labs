...
public:
  ...
  bool removeSecondLastNode(T &val) { // O(1)
    if (nodesCount < 1) {
      return false;
    } else {
  ...
  bool removeKthNode(int k, T &val) { // O(n)
    if (nodesCount == 0) {
      return false;
    } else {
      Node<T> *temp = head;
...
int main() {
  ...
  cout << "9- Delete second last node" << endl;
  cout << "10- Delete Kth node" << endl;
  cout << "11- Exit" << endl;
  cout << "\nYour choice: ";
  int choice;
  cin >> choice;
  ...
  while (choice != 11) {
    if (choice == 1) {
      ...
      int val;
      if (myList.removeSecondLastNode(val)) {
        cout << val << " removed!";
      } else {
        cout << "The list is empty or has only 1 node!";
      }
    } else if (choice == 10) {
      int val, k = -1;
      while (k < 0) {
      ...
      if (myList.removeKthNode(k, val)) {
        cout << val << " removed!";
      } else {
        cout << "The list is empty or k is larger than the total number of nodes!";
      }
    } else {
